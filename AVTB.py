...
st.set_page_config(layout="wide", page_title="AVTB-视频博客生成器", initial_sidebar_state="expanded", page_icon=":seedling:")
project_dir = os.path.dirname(os.path.abspath(__file__)).replace("\\", "/")
config = toml.load(project_dir + "/config.toml")
cache_dir = project_dir + "/output/"
st.session_state.base_url = config["GPT"]["base_url"]
st.session_state.api_key = config["GPT"]["api_key"]


with st.sidebar:
    st.write("POWERED BY @CHENYME")
    sac.divider(label='参数设置', align='center', color='gray')
    ...
    sac.divider(label='上传文件', align='center', color='gray')
...
if st.session_state.success:
    ...
    # GPT4o生成文章
    text = result['text']
    content = openai_api(api_key, base_url, "gpt-4o", "请你将下面的内容，以" + name + "的时间写成一篇文章" + text, temperature, "你是一位写作高手！")
    ...
    choose_photos = openai_api(api_key, base_url, "gpt-4o", "现在有一个视频文件，约：" + str(
        ...
        num) + "张图片。现在，请仔细阅读下面的字幕内容，根据下面的srt字幕的内容，返回你认为写一篇关于该字幕的博客的最重要的几个秒数（对应的图片，并仔细检查选择图片名称是否超过，第" + str(num) + "张），请你仔细选择最重要的图片，不要太多，因为我将会把这几个图片作为我的博客的图片内容，请给出你认为最重要的几张图片，你的回答只需给出['frame_1'，'frame_30'，'frame_46']这样的list格式！\n字幕内容：" + srt_content,
...
with st.expander("**项目介绍**", expanded=True):
    st.title("")
    st.write("本项目是一个视频生成博客的项目，你可以上传一个视频，然后通过AI生成一个博客，博客内容包括视频的文本内容和视频的关键帧图片。")
    st.title("")
        ...
        sac.steps(
            items=[
                ...
                sac.StepsItem(title='step 4', subtitle='生成博客文章', description='GPT-4o模型生成博客文章',
                              disabled=True),
            ], variant='default', placement='vertical', direction='vertical', color='dark', size='lg', index=4
        )

    with col2:
        st.write("#### 注意事项")
        st.write("")
        st.write("###### 运行后会自动在下方显示结果！")
        st.write("###### 请勿重复点击生成！")
        st.write("###### 请耐心等待结果，不要在运行时修改参数！")
        st.write("###### 大模型可能出错！")
        st.title("")
        st.title("")
        sac.buttons(items=[sac.ButtonsItem(label='来给我一颗星叭！', icon='github', href='https://github.com/Chenyme/Chenyme-AAVT'),
                           sac.ButtonsItem(label='试试我的其他项目？', icon='github', href='https://github.com/Chenyme/Chenyme-AAVT')],
